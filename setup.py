...
class CustomBuild(build_py):
    def run(self):
        ...
        # Visual Studio generators are multi-config; use the same config for build/install.
        if platform.system() == "Windows":
            build_cmd.extend(["--config", cmake_cfg])
            install_cmd.extend(["--config", cmake_cfg])
        ...
        for root, _, files in os.walk(stage_dir):
            for fn in files:
                if (fn.startswith("libdatetime") or fn == "datetime.dll") and (
                    fn.endswith(".so") or ".so." in fn or fn.endswith(".dylib") or fn.endswith(".dll")
                ):
                    libs.append(os.path.join(root, fn))

        if not libs:
            raise RuntimeError(f"No datetime runtime library found under staging dir: {stage_dir}")
